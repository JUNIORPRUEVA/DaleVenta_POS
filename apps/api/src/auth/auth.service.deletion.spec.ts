...
function buildService(
  prisma: any,
  r2: any = {
    deleteAllCompanyObjects: jest.fn().mockResolvedValue({ ok: true }),
  },
) {
...
describe("AuthService account deletion", () => {
  ...
  it("rejects account deletion when password is wrong", async () => {
    ...
    await expect(
      service.deleteAccount(userId, companyId, { password: "bad" }),
  ...
  it("requires DELETE MY COMPANY for sole-owner company deletion", async () => {
    ...
    await expect(
      service.deleteAccount(userId, companyId, {
  ...
  it("cleans storage and revokes company sessions during sole-owner deletion", async () => {
    const passwordHash = await bcrypt.hash("correct-password", 4);
    const r2 = {
      deleteAllCompanyObjects: jest
        .fn()
        .mockResolvedValue({ ok: true, deleted: 3 }),
    };
    const tx = {
    ...
    expect(tx.authSession.updateMany).toHaveBeenCalledWith({
      ...
      data: {
        ...
        revocationReason: "company_deleted",
      },
    });
  });
});
