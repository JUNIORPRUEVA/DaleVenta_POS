...
class _AppErrorOverlayState extends State<AppErrorOverlay> {
  ...
  Future<void> _showDetails(AppErrorDetails error) async {
    if (_isShowingDetails || !mounted) return;
    final navigatorContext = appRootNavigatorKey.currentContext;
    debugPrint(
      '[AppErrorOverlay] navigator available=${navigatorContext != null}',
    );
    if (navigatorContext == null) {
      debugPrint(
        '[AppErrorOverlay] No existe Navigator para mostrar el evento '
        '${error.eventId}',
      );
      return;
    }
    ...
    try {
      debugPrint('[AppErrorOverlay] showing event=${error.eventId}');
    ...
    } catch (detailsError, stack) {
      debugPrint(
        '[AppErrorOverlay] Error mostrando detalles: $detailsError\n$stack',
      );
    } finally {
